...
void SimpleVoipReceiver::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    ...
    if (currentTalkspurt.status == TalkspurtInfo::EMPTY) {
        ...
        // talkspurt continued
        // if the parameters don't match, that's because the talkspurt comes from a different module due to multicast - we just drop the packet silently
...
// multicast support added
void SimpleVoipReceiver::setSocketOptions()
{   
